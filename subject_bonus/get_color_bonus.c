...
/*   Updated: 2024/01/20 21:13:48 by hyungdki         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	bmt_vec(t_xpm_img *img, t_hit_rec *rec)
{
	...
	x = ft_clamp(x, 0, img->width - 1);
	y = ft_clamp(y, 0, img->height - 1);
	tmp = pixel_to_color(*((int *)(img->img_addr
					+ (y * img->size_line + x * (img->bpp / 8)))));
	tmp_vec = v_sub(v_mul(v_make(tmp.rr, tmp.rg, tmp.rb), 2), v_make(1, 1, 1));
	n_vec.x = rec->du.x * tmp_vec.x + rec->dv.x * tmp_vec.y
		+ rec->n_vec.x * tmp_vec.z;
	n_vec.y = rec->du.y * tmp_vec.x + rec->dv.y * tmp_vec.y
		+ rec->n_vec.y * tmp_vec.z;
	n_vec.z = rec->du.z * tmp_vec.x + rec->dv.z * tmp_vec.y
		+ rec->n_vec.z * tmp_vec.z;
