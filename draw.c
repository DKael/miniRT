...
/*   Updated: 2024/01/04 16:40:11 by hyungdki         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw(t_data *data)
{
	...
	for (int j = 0; j < data->win_size_y; ++j) {
        for (int i = 0; i < data->win_size_x; ++i) {
            pixel_center = vec_add(data->cam.pixel00_loc, vec_add(vec_multi(data->cam.pixel_delta_u, i), vec_multi(data->cam.pixel_delta_v, j)));
			tmp_ray.orig = data->cam.view_pnt;
			tmp_ray.dir = vec_sub(pixel_center, data->cam.view_pnt);
